...
function prefixedEvent(element: HTMLElement, type: string, callback: EventListenerOrEventListenerObject) {
...
function transform($e: HTMLElement, xValue: number, yValue: number, scaleValue: number, rotationValue?: number, percent?: boolean) {
  const x = xValue || 0
  const y = yValue || 0
  ...
  const transfromString = `translate(${x
    }${unit
    }, ${y
    }${unit
    }) `
...
function exolpodeGroup(x: number, y: number, trans: { scale: number; x: number; y: number; r: number }) {
...
export function sparcle(event: MouseEvent) {
